...
// Login Function.  This is to validate the user.
function login($username, $pwdSha1) {
	
	$user = new user();
	$user->__construct();
	$user->loadFromUsername(strtolower(trim($username)));
		
	// Valida el login
	$error = "";
	if ($username =! ""){
		if($user->getHashedPass() == sha1(trim($pwdSha1))){
			if($user->getUsrId() > 0){
				if($user->getActive() != '1'){
				...
				{
					$_SESSION['appUser'] = $user->getUsrId();
					$user->__destroy();
