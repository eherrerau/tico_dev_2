...
$counter = 0; // Need to change this variable name to the standard language
...
echo "<div id=\"BoxColumnsLabel\"><div id=\"engineerNameTitle\"><b>Engineer</b></div><div id=\"premier\">P</div><div id=\"especialIcons\">Bd</div><div id=\"Td\"></div><div id=\"TW\"></div><div id=\"Av\"></div><div id=\"in\">in</div><div id=\"lIn\">Lunch</div><div id=\"out\">out</div><div id=\"cases\">Cases</div></div>";
$scheduleIn = "";
$scheduleOut = "";
$scheduleLunch = "";
$scheduleLunchOff = "";
while ($row = sqlsrv_fetch_array($getUsers)) {
    ...
    while ($rowSchbyUsr = sqlsrv_fetch_array($getSchByUsr)) {
        ...
        //if (((date("H:i") >= $scheduleLunch) and (date("H:i") < $scheduleLunchOff))) {
            
        if (((date("H:i") >= $scheduleLunch->getSystemTime()) and (date("H:i") < $scheduleLunchOff->getSystemTime()))) {
            ...
            If ($rowpremier = sqlsrv_fetch_array($getpremier)) {
                ...
                if (date("Y-m-d") == date_format($row['birthday'], 'Y-m-d')) {
