...
While ($row = sqlsrv_fetch_array($result)) { //For each Engineer
    ...
    if ($rowCases = sqlsrv_fetch_array($getCases)) {
        if ($rowCases['average'] > 0) {
            ...
            $critAvg = round($rowCases['criticalAvg'], 2);
            //-------------chick if Available
            ...
            if ($rowSchbyUsr) {
                ...
                echo "<div id=row> 
                        <div id=\"engineer\"style=\"color: #FF0000;\">";
            }            
            echo " " . $row['nameToDisplay'] . "</div>
                <div id=\"tamano\">" . round($rowCases['average'], 2) . "</div>
                <div id=\"graphline\" style=\"width:" . $tamano . "px; \">";            
            if ($tamanoCritical > 0) {
                echo "<img src=\"assets/media/images/criticalBar.gif\" width=\"" . $tamanoCritical . "px\" height=\"10px\" title=\"Sev1 Avg:\" />";
            }            
            echo "</div>
                  </div>";
...
if ($totalUsers > 0) {
    ...
    "<div id=row>
        ...
        <div id=\"tamano\" color:#FF0000;\">" . round($totalAvg, 2) . "</div>
