...
function Login() {
    ...
    while ($rowglobal = sqlsrv_fetch_array($getGlobal)) {
        session_start();
        $_SESSION['DBtoUse'] = $rowglobal['DBname'];        
    }
    closeDBConnetion();
...
//    require_once("connection.php");
    ...
    if ($getNextSchedExcep === false) {
        ...
        while ($row = sqlsrv_fetch_array($getNextSchedExcep)) {
            $result[$x] = "<form action=\"include/delException.php\"  method=\"post\" id=\"delExc\" name=\"delExc\">
			<input name=\"X\" type=\"submit\" value=\"Delete\" title=\"Delete Exception\">
                        <INPUT size=\"1\" TYPE=\"text\" name=\"excId\" value=\"" . $row["schedExecId"] . "\" Style=\"visibility:hidden\"  > " . 
                        "<i class=\"icon-plane\"></i>" . $row["Reason"] . " From: " . getHoursFromDate($row["timeFrom"], "u") . " To " . getHoursFromDate($row["timeTo"], "u") . "</form> <br/>";
            $x++;
        }
    }
    return $result;
